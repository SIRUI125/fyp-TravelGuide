(function(){
    ...
    function getIndexDay(){
        ...
        for( var k = 0;k<calendarDate.days;k++ ){
            ...
            if(calendarDate.opt!=null){
                for( var d in calendarDate.opt.data ){
                    var nowDate = dataDateStr;
                    var dataDate = calendarDate.opt.data[d].date;
                    var inventory=calendarDate.opt.data[d].inventory;
                    flag = checkDate(nowDate,dataDate);
                    if( flag ){
                        if(inventory>0){
                            riliHtml += '<div class="ht-rili-td ht-rili-onclick" data-date="'+ dataDateStr +'"><span class="ht-rili-day">'+ (k + 1) +'</span><span class="ht-rili-money" data-id="'+calendarDate.opt.data[d].id+'" data-money="'+ calendarDate.opt.data[d].data +'" data-psid="'+calendarDate.opt.data[d].psid+'"data-inventory="'+calendarDate.opt.data[d].inventory+'" >&yen;'+ calendarDate.opt.data[d].data +'(剩余数量:'+calendarDate.opt.data[d].inventory+')</span></div>';
                            break;
                        }else{
                            riliHtml += '<div class="ht-rili-td ht-rili-td-disabled" data-date="'+ dataDateStr +'"><span class="ht-rili-day">'+ (k + 1) +'</span><span class="ht-rili-money" >&yen;'+ calendarDate.opt.data[d].data +'(剩余数量:'+calendarDate.opt.data[d].inventory+')</span></div>';
                            break;
                        }
    ...
    $.fn.extend({
        ...
        calendarGetActive: function(){
            ...
            var inventory=activeEle.children(".ht-rili-money").attr("data-inventory");
            var id=activeEle.children(".ht-rili-money").attr("data-id");
            return data = {
                date : date,
                money : money,
                psid:psid,
