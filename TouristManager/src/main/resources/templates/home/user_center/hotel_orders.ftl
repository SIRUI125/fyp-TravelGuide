...
<html lang="en">
            ...
            <div class="row">
              ...
              <div class="content col-lg-9 col-xs-12">
                <div class="rlr-data-container woocommerce-MyAccount-content">
                  <h1 class="type-h3 rlr-data-table__title">Your Hotel Orders</h1>
                  <div class="table">
                      ...
                      <#if pageBean.content?size gt 0>
                        ...
                        <tr class="rlr-data-table__row woocommerce-orders-table__row woocommerce-orders-table__row--status-on-hold order">
                          ...
                          <td class="rlr-data-table__cell woocommerce-orders-table__cell woocommerce-orders-table__cell-order-number" data-title="Order">
                            <img src="/photo/view?filename=${order.hotel.coverPic}" style="width:200px;height: 200pd;">
                          </td>

                          <td class="rlr-data-table__cell woocommerce-orders-table__cell woocommerce-orders-table__cell-order-number" data-title="Order">
                          <a href="/home/hotel/detail?id=${order.hotel.id}">
                            ${order.hotel.name!""}
                          </a>
                          </td>
                          ...
                          <td class="rlr-data-table__cell woocommerce-orders-table__cell woocommerce-orders-table__cell-order-actions" data-title="Actions">
                            <a href="javascript:void(0);"
                               class="rlr-data-table__link woocommerce-button button view-order-btn view"
                               data-id="${order.id}"> View
                            </a>
                            <#if order.status==0>
                              <a href="/home/hotel_order/to_pay?oid=${order.id}" class="rlr-data-table__link woocommerce-button
                               button view"
                                 data-id="${order.id}"> GoPay </a>
                        ...
                        <#else>
                          <tr>
                            <td colspan="10" style="text-align: center">No order information yet</td>
                          </tr>
                      ...
                      <#else>
                        <li class="page-item rlr-pagination__page-item disabled">
                          <a class="page-link rlr-pagination__page-link rlr-pagination__page-link--prev"
                             href="hotel_orders?&currentPage=1" aria-label="Previous">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <#else>
                              <li class="page-item rlr-pagination__page-item">
                                <a class="page-link rlr-pagination__page-link rlr-pagination__page-link--counter"
                                   href="hotel_orders?currentPage=${showPage}">${showPage}</a>
                              </li>
                            </#if>
                          </#list>
                        </ul>
                      ...
                      <#else>
                        <li class="page-item rlr-pagination__page-item">
                          <a class="page-link rlr-pagination__page-link rlr-pagination__page-link--next"
                             href="hotel_orders?currentPage=${pageBean.totalPage}" aria-label="Next">
                            <span aria-hidden="true">Next</span>
  ...
  <script type="text/javascript">
    ...
    $(".view-order-btn").click(function () {
      ...
      layer.open({
        type: 2,
        title: 'View list of line items',
        shadeClose: true,
        shade: false,
        maxmin: true, //开启最大化最小化按钮
        area: ['893px', '600px'],
        content: '/home/user_center/get_order_item?id='+id
      });
    ...
    $(".cancel_order_btn").click(function () {
      var id=$(this).attr("data-id");
      ajaxRequest('cancel_hotel_order','POST',{id:id},function (result) {
    ...
    $(".delete_order_btn").click(function () {
      var id=$(this).attr("data-id");
      ajaxRequest('delete_hotel_order','POST',{id:id},function (result) {
    ...
    layui.use(['rate','form'], function() {
      ...
      form.on('submit(comment)', function(data){
        ...
        ajaxRequest('/home/user_center/hotel_order_comment',"POST",{oid:id,score:score,content:content},function (result) {
