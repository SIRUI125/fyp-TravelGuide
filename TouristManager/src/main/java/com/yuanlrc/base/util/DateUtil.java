...
public class DateUtil {
     ...
     * @return 日期集合
     */
    public static JSONArray getCurrentMonthBetweenDates() {
        JSONArray jsonArray = new JSONArray();
        Date beginDayOfMonth = getBeginDayOfMonth(new Date());
        Date endDayOfMonth = getEndDayOfMonth(new Date());
        Calendar tempStart = Calendar.getInstance();
        tempStart.setTime(beginDayOfMonth);
        tempStart.add(Calendar.DAY_OF_YEAR, 1);
        Calendar tempEnd = Calendar.getInstance();
        tempEnd.setTime(endDayOfMonth);
        while (tempStart.before(tempEnd)) {
            JSONObject dateJson=new JSONObject();
            dateJson.put("date",DateUtil.toString(tempStart.getTime()));
            jsonArray.add(dateJson);
            tempStart.add(Calendar.DAY_OF_YEAR, 1);
        }
        return jsonArray;
    }
